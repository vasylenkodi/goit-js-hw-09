...
formElement.addEventListener("submit", (event)=>{
    ...
    for(i = 0; i < options.amount; i += 1){
        ...
        createPromise(promisePosition, promiseDelay).then(({ position , delay  })=>(0, _notiflixNotifyAio.Notify).success(`✅ Fulfilled promise ${position} in ${delay}ms`)).catch(({ position , delay  })=>(0, _notiflixNotifyAio.Notify).failure(`❌ Rejected promise ${position} in ${delay}ms`));
    }
});
