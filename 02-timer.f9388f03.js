...
const options = {
    ...
    onClose (selectedDates) {
        if (this._initialDate > selectedDates[0]) {
            (0, _notiflixNotifyAio.Notify).warning("Please choose a date in the future");
            return;
        }
...
(0, _flatpickrDefault.default)(refs.inputElement, options);
...
function setCountdown(time) {
    timerId = setInterval(()=>{
        ...
        const stopCondition = convertMs(time).seconds === 0 && convertMs(time).minutes === 0 && convertMs(time).hours === 0 && convertMs(time).days === 0;
        if (stopCondition) endCountdown(timerId);
    }, 1000);
}
function endCountdown(timerId1) {
